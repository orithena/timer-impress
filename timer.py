...
def preset_screw(time, step):
    ...
    return [ (a, math.sin(a)*200, a*300, a*300, a*9, a*9, a*9, 1, "%s:%02d" % ( ((a*step)//60), ((a*step) % 60) ),) for a in range(time//step, -1, -1) ]

@handler_for('hoparound')
def preset_hoparound(time, step):
    return [ (a, math.sin(a)*500, math.cos(a)*500, a*500, a*12, a*12, a*12, 1, "%s:%02d" % ( ((a*step)//60), ((a*step) % 60) ),) for a in range(time//step, -1, -1) ]
...
def preset_random(time, step):
    ...
    return [ (a, a*c[0], a*c[1], a*c[2], a*c[3], a*c[4], a*c[5], 1, "%s:%02d" % ( ((a*step)//60), ((a*step) % 60) ),) for a in range(time//step, -1, -1) ]

@handler_for('linear')
def preset_test(time, step):
    return [ (a, 0, a*380, a*1000, 0, 0, 0, 1, "%s:%02d" % ( ((a*step)//60), ((a*step) % 60) ),) for a in range(time//step, -1, -1) ]

@handler_for('test')
def preset_test(time, step):
    return [ (a, a*10, a*40, a*100, a*3, a*6, a*8, 1, "%s:%02d" % ( ((a*step)//60), ((a*step) % 60) ),) for a in range(time//step, -1, -1) ]

@app.route('/')
def welcome():
    return render_template('index.html', slide_params=[], presets=list(handlers.keys()))
